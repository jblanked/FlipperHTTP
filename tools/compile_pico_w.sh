#!/bin/bash
# Compile FlipperHTTP for Raspberry Pi Pico W
...
OUTPUT_DIR="$PROJECT_DIR/Raspberry Pi Pico W/C++"
...
# Raspberry Pi Pico W: Flash Size 2MB (Sketch: 1984KB, FS: 64KB), CPU Speed 200MHz
FQBN="rp2040:rp2040:rpipicow"
BUILD_PROPS="flash=2097152_65536,freq=200,ipbtstack=ipv4only"
BOARD_DEF="BOARD_PICO_W"
BOARD_VAL="6"

echo "=== FlipperHTTP Compiler for Raspberry Pi Pico W ==="
echo ""
...
[ -f "flipper-http.ino.uf2" ] && mv "flipper-http.ino.uf2" "flipper_http_pico_c++.uf2" && echo "✓ flipper_http_pico_c++.uf2"

# Remove all other files except the .uf2 and README
find . -type f ! -name "flipper_http_pico_c++.uf2" \
              ! -name "README.md" \
              -delete
