#!/bin/bash
# Compile FlipperHTTP for ESP32-S3
...
OUTPUT_DIR="$PROJECT_DIR/ESP32-S3"
...
BOARD_DEF="BOARD_ESP32_S3"
BOARD_VAL="3"

echo "=== FlipperHTTP Compiler for ESP32-S3 ==="
echo ""
...
[ -f "flipper-http.ino.partitions.bin" ] && mv "flipper-http.ino.partitions.bin" "flipper_http_partitions_esp32_s3.bin" && echo "✓ flipper_http_partitions_esp32_s3.bin"
[ -f "flipper-http.ino.bootloader.bin" ] && mv "flipper-http.ino.bootloader.bin" "flipper_http_bootloader_esp32_s3.bin" && echo "✓ flipper_http_bootloader_esp32_s3.bin"
[ -f "flipper-http.ino.bin" ] && mv "flipper-http.ino.bin" "flipper_http_firmware_a_esp32_s3.bin" && echo "✓ flipper_http_firmware_a_esp32_s3.bin"
[ -f "flipper-http.ino.merged.bin" ] && mv "flipper-http.ino.merged.bin" "flipper_http_merged_esp32_s3.bin" && echo "✓ flipper_http_merged_esp32_s3.bin"

find . -type f ! -name "flipper_http_partitions_esp32_s3.bin" \
              ! -name "flipper_http_bootloader_esp32_s3.bin" \
              ! -name "flipper_http_firmware_a_esp32_s3.bin" \
              ! -name "flipper_http_merged_esp32_s3.bin" \
              ! -name "README.md" \
              -delete
