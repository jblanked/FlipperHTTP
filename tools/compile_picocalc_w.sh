#!/bin/bash
# Compile FlipperHTTP for PicoCalc W (Raspberry Pi Pico W with TFT display)
# macOS: brew update && brew install arduino-cli
# NOTE: Requires TFT_eSPI library configured per Development.md
...
# PicoCalc W uses Raspberry Pi Pico W: Flash Size 2MB, CPU Speed 200MHz
FQBN="rp2040:rp2040:rpipicow"
BUILD_PROPS="flash=2097152_65536,freq=200,ipbtstack=ipv4only"
BOARD_DEF="BOARD_PICOCALC_W"
BOARD_VAL="12"

echo "=== FlipperHTTP Compiler for PicoCalc W ==="
echo ""
echo "NOTE: This requires TFT_eSPI library configured per Development.md"
echo ""
...
arduino-cli lib install "ArduinoJson" "ArduinoHttpClient" "TFT_eSPI" || true
...
arduino-cli compile \
    --fqbn "$FQBN" \
    --build-property "build.extra_flags=-DPICO_CYW43_SUPPORTED=1 -DCYW43_PIN_WL_DYNAMIC=1 -D$BOARD_DEF=$BOARD_VAL" \
...
[ -f "flipper-http.ino.uf2" ] && mv "flipper-http.ino.uf2" "flipper_http_picocalc_w.uf2" && echo "✓ flipper_http_picocalc_w.uf2"

# Remove all other files except the .uf2 and README
find . -type f ! -name "flipper_http_picocalc_w.uf2" \
              ! -name "flipper_http_picocalc_2w.uf2" \
              ! -name "README.md" \
              -delete
