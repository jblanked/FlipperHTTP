#!/bin/bash
# Compile FlipperHTTP for Video Game Module (Raspberry Pi Pico)
...
BUILD_PROPS="flash=2097152_65536,freq=200"
BOARD_DEF="BOARD_VGM"
BOARD_VAL="8"

echo "=== FlipperHTTP Compiler for Video Game Module ==="
echo ""
...
arduino-cli compile \
    --fqbn "$FQBN" \
    --build-property "build.extra_flags=-D$BOARD_DEF=$BOARD_VAL -DCORE_DEBUG_LEVEL=0 -Os" \
...
[ -f "flipper-http.ino.uf2" ] && mv "flipper-http.ino.uf2" "flipper_http_vgm_c++.uf2" && echo "✓ flipper_http_vgm_c++.uf2"

# Remove all other files except the .uf2 and README
find . -type f ! -name "flipper_http_vgm_c++.uf2" \
              ! -name "README.md" \
              -delete
