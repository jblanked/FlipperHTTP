#!/bin/bash
# Compile FlipperHTTP for ESP32-Cam (ESP32-WROOM-DA Module)
...
OUTPUT_DIR="$PROJECT_DIR/ESP32-Cam"
...
echo "=== FlipperHTTP Compiler for ESP32-Cam ==="
echo ""
...
[ -f "flipper-http.ino.partitions.bin" ] && mv "flipper-http.ino.partitions.bin" "flipper_http_partitions_esp32_cam.bin" && echo "✓ flipper_http_partitions_esp32_cam.bin"
[ -f "flipper-http.ino.bootloader.bin" ] && mv "flipper-http.ino.bootloader.bin" "flipper_http_bootloader_esp32_cam.bin" && echo "✓ flipper_http_bootloader_esp32_cam.bin"
[ -f "flipper-http.ino.bin" ] && mv "flipper-http.ino.bin" "flipper_http_firmware_a_esp32_cam.bin" && echo "✓ flipper_http_firmware_a_esp32_cam.bin"
[ -f "flipper-http.ino.merged.bin" ] && mv "flipper-http.ino.merged.bin" "flipper_http_merged_esp32_cam.bin" && echo "✓ flipper_http_merged_esp32_cam.bin"

find . -type f ! -name "flipper_http_partitions_esp32_cam.bin" \
              ! -name "flipper_http_bootloader_esp32_cam.bin" \
              ! -name "flipper_http_firmware_a_esp32_cam.bin" \
              ! -name "flipper_http_merged_esp32_cam.bin" \
              ! -name "README.md" \
              -delete
